...
parser.add_argument('-b', default=128, type=int, metavar='b', help='batchSize')
...
testloader = torch.utils.data.DataLoader(testset, batch_size=int(args.b/2.), shuffle=False, num_workers=2)
...
# define leminiscate
lemniscate = NCEAverage(args.low_dim, ndata, args.nce_t, args.nce_m, batchSize=args.b)
...
if args.test_only:
    acc = kNN(0, net, lemniscate, trainloader, testloader, 200, args.nce_t, 1, c=args.c)
    sys.exit(0)
...
def adjust_learning_rate(optimizer, epoch):
    ...
    if epoch < 5:
        lr = lr *10**(-2)
        lemniscate.update_m(0.9)
    else:
        lemniscate.update_m(args.nce_m)
